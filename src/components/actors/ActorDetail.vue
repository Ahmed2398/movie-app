<template>
  <div>
    <div class="container mx-auto flex mt-20 border-b border-gray-600 pb-2">
      ...
      <div class="ml-20">
        <h2 class=" text-4xl font-bold	 uppercase mb-2 ">
          {{ this.actor.name }}
        </h2>
 <h4 class=" text-4xl font-bold text-orange-500	 uppercase mb-2 ">
          {{ this.actor.known_for_department }}
        </h4>
        <div class="flex mt-2">
          <svg
          ...
          <span class="ml-2 text-gray-400 text-sm">{{this.actor.place_of_birth}}</span>
        </div>
        <p class="  lowercase text-lg	 font-semibold leading-normal text-justify text-zinc-400  mt-8">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 ">
          <div :key="movie.id" v-for="movie in this.knownFor">
            ...
            >
            <div class="p-3  w-full hover:opacity-70 transition easy-in-out duration-150 ">
       <div class="bg-white rounded-lg shadow-lg">
              <img
                :src="movieImage(movie)"
                alt="poster"
                class="mt-1 hover:opacity-75 transition ease-in-out duration-150" style="width:100%"
              />
                 <h2 class="font-bold mb-2 text-1xl text-purple-800 py-5 text-center"> {{ movie.name }}</h2>
        ...
        <ul class="list-disc leading-loose pl-5 mt-8">
          <li :key="cast.id" v-for="cast in castMovies">
            ...
            <router-link :to="`/movie/${cast.id}`" class="hover:underline">
             <span class=" text-2l font-bold text-gray-500 mb-2"> {{ cast.title }}</span>
            </router-link>
            as<span class=" text-2l font-bold text-orange-500 mb-2"> {{ cast.character }}</span>
          </li>
        </ul>
      </div>
    </div>
...
export default {
 ...
 methods:{
   async fetchActor(actorId){
        const response = await this.$http.get("/person/" + actorId + "?api_key=8c0c4f0ffe0491beaa6e0c8fa562fe16" );
        this.actor = response.data;
        
    },
    async fetchCredits(actorId) {
      const response = await this.$http.get(
        "https://api.themoviedb.org/3/person/" + actorId + "/combined_credits?api_key=8c0c4f0ffe0491beaa6e0c8fa562fe16"
      );
    ...
    async fetchSocial(actorId) {
      const response = await this.$http.get(
        "https://api.themoviedb.org/3/person/" + actorId + "/external_ids?api_key=8c0c4f0ffe0491beaa6e0c8fa562fe16"
      );
